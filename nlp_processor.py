...
class HeartDiseaseNLPExtractor:
   ...
   def _extract_oldpeak(self, text: str) -> Optional[float]:
        ...
        for pattern in patterns:
            ...
            if matches:
                try:
                    op = float(matches[0])
                    return min(max(op, 0.0), 6.0)  # Giới hạn hợp lý
                except:
                    continue

        # Nếu có triệu chứng nặng, ước tính oldpeak cao hơn
   ...
   def update_features_with_response(self, features: Dict[str, Any],
                                     response_text: str,
